...
def vector_search_agent(state: State):
    ...
    msg_str = f"[VECTOR SEARCH AGENT] 다음 질문에 대한 검색 완료: {query}"
    ...
    # communicator
    ai_recommendation = "현재 참고자료(references)가 목차(outline)를 개선하는 데 충분한지 확인하라. 충분하다면 content_strategist로 목차 작성을 하라."
...
def content_strategist(state: State):
    ...
    if task.agent != "content_strategist":
        raise ValueError(f"Content Strategist가 아닌 agetn가 목차 작성을 시도하고 있습니다.\n {task}")
    ...
    content_strategist_system_prompt = PromptTemplate.from_template(
        ...
        목차를 작성하는 데 필요한 정보는 "참고자료"에 있으므로 활용한다.
        ...
        너의 작업 목표는 다음과 같다:
        1. 만약 "기존 목차 구조(previous_outline)"이 존재한다면, 사용자의 요구 사항을 토대로 "기존 목차 구조"에서 어떤 부분을 수정하거나 추가할지를 결정한다.
        ...
        6. 참고 문헌은 반드시 참고자료(references) 자료를 근거로 작성해야 하며, 최대한 풍부하게 준비한다. URL은 전체 주소를 적어야 한다.
        ...
        outline_template은 예시로 제시한 것이다. 예시에 없는 항목이 있다면 추가하라.
        outline_template:
        {outline_template}
...
def outline_reviewer(state: State):
    ...
    outline_reviewer_system_prompt = PromptTemplate.from_template(
        """
        너는 AI팀의 목차 리뷰어로서, AI팀이 작성한 목차(outline)을 검토하고 문제점을 지적한다.
    ...
    if '[OUTLINE REVIEWER]' not in gathered.content:
...
def communicator(state: State):
    ...
    # gathered를 AIMessage로 변환하여 추가
    ai_message = AIMessage(content=gathered.content)
...
# 그래프 도식화
try:
    # Mermaid 다이어그램을 텍스트로 저장
    mermaid_code = graph.get_graph().draw_mermaid()
    
    # Mermaid 코드를 파일로 저장
    mermaid_file_path = absolute_path.replace('.py', '_graph.mmd')
    with open(mermaid_file_path, 'w', encoding='utf-8') as f:
        f.write(mermaid_code)
    
    print(f"그래프 다이어그램이 성공적으로 생성되었습니다: {mermaid_file_path}")
    print("Mermaid 코드를 https://mermaid.live 에서 확인할 수 있습니다.")
    
except Exception as e:
    print(f"그래프 다이어그램 생성 중 오류 발생: {e}")
    print("다이어그램 생성은 건너뛰고 계속 진행합니다.")
